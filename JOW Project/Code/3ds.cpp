...
void CLoad3DS::ProcessNextChunk(t3DModel *pModel, tChunk *pPreviousChunk)
{
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		case VERSION:							// This holds the version of the file
			...
			currentChunk.bytesRead += (UINT)fread(gBuffer, 1, currentChunk.length - currentChunk.bytesRead, m_FilePointer);
		...
		case OBJECTINFO:						// This holds the version of the mesh
			...
			tempChunk.bytesRead += (UINT)fread(gBuffer, 1, tempChunk.length - tempChunk.bytesRead, m_FilePointer);
		...
		// WE DO NOT CARE ABOUT MATERIALS
...
void CLoad3DS::ProcessNextObjectChunk(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		case OBJECT_LOCAL:					// This holds the Local Axis Information of the Object

			// This chunk holds the Axis information of the object
			ReadAxis(pObject, &currentChunk);
			break;
...
void CLoad3DS::ProcessNextMaterialChunk(t3DModel *pModel, tChunk *pPreviousChunk)
{
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		case MATNAME:							// This chunk holds the name of the material
			...
			currentChunk.bytesRead += (UINT)fread(pModel->pMaterials[pModel->numOfMaterials - 1].strName, 1, currentChunk.length - currentChunk.bytesRead, m_FilePointer);
			break;
		...
		case MATMAPFILE:						// This stores the file name of the material
			...
			currentChunk.bytesRead += (UINT)fread(pModel->pMaterials[pModel->numOfMaterials - 1].strFile, 1, currentChunk.length - currentChunk.bytesRead, m_FilePointer);
			break;
...
void CLoad3DS::ReadColorChunk(tMaterialInfo *pMaterial, tChunk *pChunk)
{
	...
	tempChunk.bytesRead += (UINT)fread(pMaterial->color, 1, tempChunk.length - tempChunk.bytesRead, m_FilePointer);
...
void CLoad3DS::ReadUVCoordinates(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	UINT _tCount = pObject->numTexVertex;

	// Allocate memory to hold the UV coordinates
	pObject->pTexVerts = new CTexCoord[_tCount];
	memset(pObject->pTexVerts, 0, sizeof(CTexCoord) * _tCount);

	// You need to read in the information into a float array
	float* _temp = new float[_tCount*2];
	memset(_temp, 0, sizeof(float) * _tCount*2);

	// Read in the texture coodinates (an array 2 float)
	pPreviousChunk->bytesRead += (UINT)fread(_temp, 1, pPreviousChunk->length - pPreviousChunk->bytesRead, m_FilePointer);

	// Copy Values
	for(UINT i = 0; i < _tCount; i++)
	{		
		pObject->pTexVerts[i].u =  _temp[i*2 + 0];	
		pObject->pTexVerts[i].v =  _temp[i*2 + 1];	
	}
	delete[] _temp;
}
...
///////////////////////////////// READ AXIS \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This function reads the local axis of the object
/////
///////////////////////////////// READ AXIS \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*

void CLoad3DS::ReadAxis(t3DObject *pObject, tChunk *pPreviousChunk)
{
	//	Local axis info.
	...
	// TODO :: Figure out what to do with Axis Information
...
void CLoad3DS::ReadVertices(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	UINT _vCount = pObject->numOfVerts;

	// Allocate the memory for the verts and initialize the structure
	pObject->pVerts = new CVector3 [_vCount];
	memset(pObject->pVerts, 0, sizeof(CVector3) * _vCount);
	...
	// Go through all of the vertices that we just read
	for(UINT i = 0; i < _vCount; i++)
	{		
		pObject->pVerts[i].x =  _temp[i*3 + 0];
		pObject->pVerts[i].y =  _temp[i*3 + 1];
		pObject->pVerts[i].z =  _temp[i*3 + 2];
	}
...
void CLoad3DS::ReadObjectMaterial(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	pPreviousChunk->bytesRead += (UINT)fread(gBuffer, 1, pPreviousChunk->length - pPreviousChunk->bytesRead, m_FilePointer);
}			
...
void CLoad3DS::ComputeNormals(t3DModel *pModel)
{
	...
	for(int index = 0; index < pModel->numOfObjects; index++)
	{
		...
		{												
			...
			vPoly[0] = pObject->pVerts[pObject->pFaces[i].v[0]];
			vPoly[1] = pObject->pVerts[pObject->pFaces[i].v[1]];
			vPoly[2] = pObject->pVerts[pObject->pFaces[i].v[2]];
			...
			vNormal  = vVector1* vVector2;				// Return the cross product of the 2 vectors (normalize vector, but not a unit vector)
		...
		for (i = 0; i < pObject->numOfVerts; i++)			// Go through all of the vertices
		{
			...
			{												// Check if the vertex is shared by another face
				if (pObject->pFaces[j].v[0] == i || 
					pObject->pFaces[j].v[1] == i || 
					pObject->pFaces[j].v[2] == i)
				{
...
//
// Not strange and ugly it makes sense to me and thats how my co-ord system works as well.
// Therefore there is no need to swap araound.
//
